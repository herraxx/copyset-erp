...
(function(){
 ...
 function show(view){if(!titles[view])return false;document.querySelectorAll('.nav').forEach(x=>x.classList.toggle('on',x.dataset.v===view));document.querySelectorAll('[data-top-v]').forEach(x=>x.classList.toggle('on',x.dataset.topV===view));document.querySelectorAll('.view').forEach(x=>x.classList.toggle('on',x.id===view));const title=document.getElementById('title');if(title)title.textContent=titles[view];history.replaceState(null,'','#'+view);return true}
 ...
 document.querySelectorAll('[data-top-v]').forEach(b=>b.addEventListener('click',()=>show(b.dataset.topV)));
