...
(function(){
 ...
 function open(order,options={}){const m=modal(),b=body(),t=title(),renderer=root.orderFormRenderer;if(!m||!b||!t||!renderer)return false;const modeValue=options.mode||order?.mode||'Tarjous';legacySetContext(order?.id||null,modeValue);const html=renderer.render(order);if(!html)return false;t.textContent=order?.id?`Muokkaa ${order.no||''}`:(modeValue==='Tarjous'?'Uusi tarjous':'Uusi tilaus');b.innerHTML=html;m.classList.add('on');renderer.bind(b);queueMicrotask(()=>root.formLifecycle?.announce?.());return true}
