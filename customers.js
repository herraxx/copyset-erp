...
(function(){
  ...
  const aliases={
    company:['customerCompany','company','customerName'],contact:['customerContact','contact','contactPerson'],
    bid:['customerBid','businessId','bid','ytunnus'],email:['customerEmail','email'],phone:['customerPhone','phone'],
    address:['customerAddress','address'],zip:['customerZip','postalCode','zip','postinumero'],city:['customerCity','city'],country:['customerCountry','country']
  };
  ...
  function fillForm(form,customer){const row=normalize(customer);for(const [key,names] of Object.entries(aliases)){const el=field(form,names);if(el&&key in row)el.value=row[key]??''}form.dispatchEvent(new CustomEvent('copyset:customer-filled',{bubbles:true,detail:{customer:row}}));return row}
